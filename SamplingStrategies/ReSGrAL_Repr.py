...
# Function to perform active learning using ReSGrAL Uncertainty sampling
def active_learning_repr_prof_prof(X, y, S, n_start, n_instances, distance_metric, profiles, seed, fairness_metric, n_queries=500, model_mode = LogisticRegression):
    ...
    np.random.seed(42)  # Set a seed value for reproducibility
    ...
    # calculate all the representativenes scores per profile
    profile_scores = {}
    for profile_idx, data_instances in profile_data.items():
        data_instances = np.array(data_instances)
        info_density = information_density(data_instances, metric=similarity)
        score_index = sorted(range(len(info_density)), key=lambda k: info_density[k])
        profile_scores[profile_idx] = score_index
    ...
    while total_instances < n_queries + n_start:
        ...
        # for every cluster, pick the most representative
        for profile_idx, score_index in profile_scores.items():
            ...
            query_idx = score_index[:n_instances]
            query_inst = [profile_data[profile_idx][query_idx[0]]]
            ...
            # make sure that the next score is taken
            profile_scores[profile_idx] = score_index[n_instances:]
...
def plot_performance_repr_prof_profiles(dataset_name, df, y_label, S, n_experiments, distance_metric, profiles, fairness_metric, n_queries, random_number, set_seed=42, model_mode=LogisticRegression):
    ...
    for i in range(n_experiments):
        ...
        performance_history, fairness_history, f1_score_history, instances, performance_history_profiles, fairness_history_profiles, f1_scores_history_profiles = \
                                                active_learning_repr_prof_prof(df.values, y_label.values, S.values, 8, 1, \
                                                                               distance_metric, \
                                                                               profiles, random_numbers[i], 
                                                                               fairness_metric, n_queries, model_mode=model_mode)
    ...
    save_dict_to_csv(performance_histories_profiles, 'Results/' + str(dataset_name) + 'ReSGrAL_repr' + str(distance_metric.__name__) + str(random_number) + 'experiments_accuracy_histories_profiles_' + str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
    save_dict_to_csv(performance_histories, 'Results/' + str(dataset_name) + 'ReSGrAL_repr' + str(distance_metric.__name__) + str(random_number) + 'experiments_accuracy_histories_total_' + str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')

    # save the results - Fairness
    save_dict_to_csv(fairness_histories_profiles, 'Results/' + str(dataset_name) + 'ReSGrAL_repr' + str(distance_metric.__name__) + str(random_number) + 'experiments_fairness_histories_profiles_' + str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
    save_dict_to_csv(fairness_histories, 'Results/' + str(dataset_name) + 'ReSGrAL_repr' + str(distance_metric.__name__) + str(random_number) + 'experiments_fairness_histories_total_' +  str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')

    # save the results - F1-score
    save_dict_to_csv(f1_scores_histories_profiles, 'Results/' + str(dataset_name) + 'ReSGrAL_repr' + str(distance_metric.__name__) + str(random_number) +  'experiments_f1_histories_profiles_' +  str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
    save_dict_to_csv(f1_scores_histories, 'Results/' + str(dataset_name) + 'ReSGrAL_repr' + str(distance_metric.__name__) + str(random_number) + 'experiments_f1_histories_total_' +  str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
...
if __name__ == "__main__":
    ...
    ###########################
    plot_performance_repr_prof_profiles(dataset_name, df, y_label, S, 1, euclidean, profiles, demographic_parity_difference, n_samples, random_number, set_seed=42, model_mode = RandomForestClassifier)
    end_time = time.time()
