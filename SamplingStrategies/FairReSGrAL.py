...
# def process(chunk_id, chunk, Xl, yl, X_test, S_test, y_test, fairness_metric, model_mode):
def process(chunk_id, chunk):
    ...
    for i in range(0, len(chunk)):
        ...
        for j in range(0, 2):
            ...
            proba = model_tmp.predict_proba([chunk[i]])
            if len(proba == 1):
                if j == 0:
                    p = 1
                else:
...
def active_learning_threshold_fal(X, y, S, n_start, n_instances, query_strategy, profiles, seed, fairness_metric, threshold = 0.5, n_queries=500, model_mode=LogisticRegression):
    ...
    profiles_labeled = profiles_v_train[random_indices]
    ...
    # Group unlabeled data into profiles
    profile_unlabeled_data = {k : [] for k in profiles.unique()}
    for i, profile_idx in enumerate(profiles_train_pool):
        profile_unlabeled_data[profile_idx].append(list(X_train_pool[i]))
        
        
    # Group labeled data and labels into profiles
    profile_labeled_data = {k : [] for k in profiles.unique()}
    profile_labels = {k : [] for k in profiles.unique()}
    for i, profile_idx in enumerate(profiles_labeled):
        profile_labeled_data[profile_idx].append(list(X_labeled[i]))
        profile_labels[profile_idx].append(y_labeled[i])
    
    # group all the true labels into profiles
    profile_true_labels = {k : [] for k in profiles.unique()}
    for i, profile_idx in enumerate(profiles_train_pool):
        profile_true_labels[profile_idx].append(y_train_pool[i])
    
    # Group test data into profiles
    profile_test_data = profile_data_dict(profiles_test, X_test)
    # group the true test labels into profiles
    profile_test_labels = profile_labels_dict(profiles_test, y_test)
    # group the sensitive attributes into profiles
    profile_test_S = profile_S_dict(profiles_test, S_test)
    ...
    while total_instances < n_queries + n_start:
        print('query:', total_instances, n_queries + n_start)
        # choose next datasample
        for profile_idx, data in profile_unlabeled_data.items():
            ...
            if fairness_history_profiles[profile_idx][-1] > threshold:
                # FAL sampling needs at least 1 labeled instance of the group
                if len(profile_labeled_data[profile_idx]) > 0:
                    FAL_sampling += 1
                    query_idx, query_inst = FAL_sampling_parallel(profile_labeled_data[profile_idx], data,
                                                                profile_labels[profile_idx], profile_test_data[profile_idx],
                                                                profile_test_S[profile_idx], profile_test_labels[profile_idx],
                                                                fairness_metric)
                else:
                    # do AL sampling
                    normal_sampling += 1
            ...
            query_labels = profile_true_labels[profile_idx][query_idx[0]]

            # add to the new labeled pool per profile
            profile_labeled_data[profile_idx].append(profile_unlabeled_data[profile_idx][query_idx[0]])
            profile_labels[profile_idx].append(profile_true_labels[profile_idx][query_idx[0]])

            # Remove the queried instance from the unlabeled pool per profile
            del profile_unlabeled_data[profile_idx][int(query_idx[0])]
            del profile_true_labels[profile_idx][int(query_idx[0])]
...
def plot_performance_threshold_fal(dataset_name, df, y_label, S, n_experiments, query_strategy, profiles, fairness_metric, threshold, set_seed=42, n_queries=40, model_mode=LogisticRegression):
    ...
    for i in range(n_experiments):
        print('experiment ', i)
        print('random_numbers[i]', random_numbers[i])
        performance_history, fairness_history, f1_score_history, instances, performance_history_profiles, fairness_history_profiles, f1_scores_history_profiles = \
                                active_learning_threshold_fal(df.values, y_label.values, S.values, 16, 1, \
                                                            query_strategy, profiles, random_numbers[i], fairness_metric, \
                                                            threshold = threshold, n_queries=n_queries, model_mode=model_mode)
    ...
    save_dict_to_csv(performance_histories_profiles, 'Results/' + str(dataset_name) + str(threshold) + 'T_FAL_total_LG_' + str(random_number) + 'experiments_accuracy_histories_profiles_' + str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
    save_dict_to_csv(performance_histories, 'Results/' + str(dataset_name) + str(threshold) + 'T_FAL_total_LG_' + str(random_number) + 'experiments_accuracy_histories_total_' + str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')

    # save the results - Fairness
    save_dict_to_csv(fairness_histories_profiles, 'Results/' + str(dataset_name) + str(threshold) + 'T_FAL_total_LG_' + str(random_number) + 'experiments_fairness_histories_profiles_' + str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
    save_dict_to_csv(fairness_histories, 'Results/' + str(dataset_name) + str(threshold) + 'T_FAL_total_LG_' + str(random_number) + 'experiments_fairness_histories_total_' +  str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')

    # save the results - F1-score
    save_dict_to_csv(f1_scores_histories_profiles, 'Results/' + str(dataset_name) + str(threshold) + 'T_FAL_total_LG_' + str(random_number) + 'experiments_f1_histories_profiles_' +  str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
    save_dict_to_csv(f1_scores_histories, 'Results/' + str(dataset_name) + str(threshold) + 'T_FAL_total_LG_' + str(random_number) + 'experiments_f1_histories_total_' +  str(len(np.unique(profiles))) + 'profiles' + str(n_queries) +'iter_'+'.csv')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 8:
        ...
        print(f"and threshold: {threshold}")
    else:
        print(f"Not enough command line arguments. Please provide all 7 prompts.")
        exit()
    ...
    ###########################
    plot_performance_threshold_fal(dataset_name, df, y_label, S, 1, uncertainty_sampling, profiles, demographic_parity_difference, threshold, set_seed=random_number, n_queries=n_samples, model_mode=RandomForestClassifier)
    end_time = time.time()
